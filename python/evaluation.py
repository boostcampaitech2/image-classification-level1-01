...
def evaluation(args):
    ...
    valid_dataloader = DataLoader(
        valid_test_data, batch_size=16, shuffle=False, num_workers=4)
    ...
    with torch.no_grad():
        for sample in tqdm.tqdm(valid_dataloader):
            ...
            for mask, age, gender, label_mask, label_age, label_gender in zip(preds_mask, preds_age, preds_gender,sample['label_mask'], sample['label_age'], sample['label_gender']):
                ans = list()
                label = list()
