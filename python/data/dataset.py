...
class magv_dataset(Dataset):
	def __init__(
			...
			age_test_num=None, model=None
			):
		self.index = index
		self.mask_data = prepare_mask_data(data_path)
		self.age_gender_data = prepare_age_gender_data(
				self.mask_data, age_test_num
				)
		self.data = concat_data(
				self.mask_data, self.age_gender_data
				).iloc[self.index]
